...
During `wire_e2e_setup.sh`, the fixture sources are copied to an external staging directory under `$TMPDIR`. The script renames `.gleam.template` to `.gleam` and maps the template directories to the conventional Gleam paths (`shared/src/`, `src/`, `clients/web/src/`). Building the fixture in-place would create a `build/` directory visible to gleam when compiling libero as a path dependency, causing duplicate native Erlang module errors.

### Files

- `gleam.toml` — libero as a path dependency, declares the `web` client

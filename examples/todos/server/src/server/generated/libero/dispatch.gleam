...
pub fn handle(
  ...
  case wire.decode_call(data) {
    Ok(#("shared/todos", msg)) ->
      dispatch(state, fn() {
        server_store_handler.update_from_client(msg: wire.coerce(msg), state:)
      })
    Ok(#(name, _)) -> #(
...
fn dispatch(
  ...
  case trace.try_call(call) {
    Ok(Ok(#(value, new_state))) ->
      ...
      #(
        wire.tag_response(wire.encode(Ok(wire.unwrap_response(value)))),
        None,
        new_state,
      )
    Ok(Error(app_err)) -> #(
      wire.tag_response(wire.encode(Error(error.AppError(app_err)))),
      None,
      state,
    )
    Error(reason) -> {

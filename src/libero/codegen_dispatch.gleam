...
pub fn write_endpoint_dispatch(
  ...
  // Generate case arms for dispatch routing. Mutating handlers (return
  // shape `#(Result, HandlerContext)`) are invoked directly; read-only
  // handlers (bare `Result`) are wrapped to thread the inbound context
  // through unchanged, so dispatch always sees a `#(_, HandlerContext)`.
  let case_arms =
    list.map(endpoints, fn(e) {
      ...
      let raw_call = alias <> "." <> e.fn_name <> "(" <> handler_args <> ")"
      let invocation = case e.mutates_context {
        True -> raw_call
        False -> "#(" <> raw_call <> ", handler_ctx)"
      }
      "        "
      <> param_destructure

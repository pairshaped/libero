...
This guide walks you from an empty directory to a working todo app: SQLite on the server, typed RPC over WebSocket, and a Lustre SPA in the browser. Every command and every file is shown. No hand-waves.
...
- `examples/todos` in the libero repo: the same shape but with in-memory storage instead of SQLite. Useful when you want to compare the two storage approaches side by side.
- `examples/default`: the bare scaffold this guide started from. Useful when you want a non-todos starting point.
